...
const {strToInt, createJoinObject, createEvent, parseJobInfo} = require('./triggers/functions');
...
const buildsPlugin = {
    ...
    async register(server, options) {
        ...
        server.expose('triggerNextJobs', async (config, app) => {
            const {pipeline: currentPipeline, job: currentJob, build: currentBuild} = config;
            ...
            const pipelineJoinData = await createJoinObject(
                nextJobsTrigger,
                current,
                eventFactory,
            );
